...
def foo(qvars: QVars):
    ...
    # We need to compute the sum and apply phase
    total = qvars.x + qvars.y
    # Apply a phase rotation based on the total
    RZ(np.pi * total / 8, total)

# Main function that initializes qstruct and applies H transforms
@qfunc
def main(qvars: Output[QVars]):
    # Initialize the qstruct with allocate for each qnum
    allocate(3, qvars.x)
    allocate(3, qvars.y)
