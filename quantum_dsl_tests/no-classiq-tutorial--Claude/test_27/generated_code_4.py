...
def diffuser(a: QNum, b: QNum, c: QNum):
    ...
    # Apply phase flip to |000> state
    control(a == 0, lambda: control(b == 0, lambda: control(c == 0, lambda: PHASE(np.pi))))
...
def grover_iteration(a: QNum, b: QNum, c: QNum):
    """
    Single Grover iteration: oracle + diffuser
    """
    phase_oracle(a, b, c)
    diffuser(a, b, c)
...
if __name__ == "__main__":
    ...
    for state, amplitude in results.parsed_counts.items():
        ...
        print(f"a={a_val}, b={b_val}, c={c_val} -> {final} {'== 4 ✓' if final == 4 else '!= 4'} (probability: {amplitude})")