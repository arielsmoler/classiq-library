...
def main():
    ...
    hamiltonian = PauliOperator(pauli_terms)
    
    @qfunc
    def main_circuit(qubits: QArray[QBit]):
    ...
    model = create_model(main_circuit, out_wires_count=4)
    ...
    print(f"Circuit depth: {quantum_program.data.depth}")
    
    # Execute the circuit
    execution_preferences = ExecutionPreferences(num_shots=1024)
    job = execute(quantum_program, preferences=execution_preferences)
    results = job.result()
