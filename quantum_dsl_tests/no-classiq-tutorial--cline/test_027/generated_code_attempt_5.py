...
@qfunc
def grover_diffuser(a: QNum, b: QNum, c: QNum):
    """
    Grover diffusion operator (inversion about average)
    """
    within_apply(
        lambda: [hadamard_transform(a), hadamard_transform(b), hadamard_transform(c)],
        lambda: within_apply(
            lambda: [invert(a), invert(b), invert(c)],
            lambda: [
                # Multi-controlled Z gate on the last qubit
